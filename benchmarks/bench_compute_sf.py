...
if __name__ == "__main__":
    ...
    for n in range(4,total + 1):
        ...
        print("n-cycle - Mosek...")
        res = timeit.timeit("em.compute_sf(solver='MOSEK')", setup=f"from __main__ import generate_random_em_ncycle; em = generate_random_em_ncycle({n})", number=100)
        results["n-cycle"]["MOSEK"][n] = res
        print("Done")
        
        print("n-cycle - HiGHS...")
        res = timeit.timeit("em.compute_sf(solver='HiGHS')", setup=f"from __main__ import generate_random_em_ncycle; em = generate_random_em_ncycle({n})", number=100)
        results["n-cycle"]["HiGHS"][n] = res
        print("Done")
        
        print("random - Mosek...")
        res = timeit.timeit("em.compute_sf(solver='MOSEK')", setup=f"from __main__ import generate_random_em_random; em = generate_random_em_random({n})", number=100)
        results["random"]["MOSEK"][n] = res
        print("Done")
        
        print("random - HiGHS...")
        res = timeit.timeit("em.compute_sf(solver='HiGHS')", setup=f"from __main__ import generate_random_em_random; em = generate_random_em_random({n})", number=100)
        results["random"]["HiGHS"][n] = res
        print("Done")
        
        gc.collect()
    
    with open('data_sf.json', 'w+') as f:
